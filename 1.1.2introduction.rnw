...
Today's standard model of small tree growth, the Forest Vegetation Simulator, introduces variability in height growth increment through assigning average growth from three different intervals within an assumed normal distribution about a mean height increment prediction. This parametric technique to introduce variability in estimates 
