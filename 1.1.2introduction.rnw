...
Understory vegetation and overstory tree density both provide conflicting facilitating and competitive effects on ponderosa pine regeneration growth and survival in the understory (\cite{Keyes2005}). The shading provided by the retained overstory trees helps prevent dessication in the summer months and the understory vegetation provides similar reductions in moisture stress, although within the microenvironment. However, excessive shade and accelerated soil moisture depletion may reduce small tree growth. Partial retention harvests may provide a balance of the conflicting effects of overstory retention and understory vegetation, as ponderosa pine regeneration responds positively to openings in the canopy, particularly on moisture limited sites (\cite{Harrod2009}, \cite{Aubry2009}).

Predictive modeling of small ponderosa pine growth dynamics following such partial retention harvest treatments can be very challenging however, particularly relative to even-aged stands, as it requires consideration of complex competitive effects from both the retained overstory and the understory vegetation.

%Basic Silvics of Ponderosa Pine
\subsection{Ponderosa Pine Silviculture Systems}
Ponderosa pine is an important species both ecologically and commercially in the Inland Northwest region of North America which has been managed by a broad array of silvicultural systems are applied to ponderosa pine stands in the Northern Rocky Mountains, including group selection, seed tree, or shelterwood systems (\cite{adams_1994}). Multiple-entry management using either long-rotation even-aged systems or uneven-aged systems are suggested, with group selection being the most highly recommended (\cite{tesch_1994}). These groups of retained trees provide openings with sufficient light to allow shade intolerant species to germinate and be competitive in mixed species stands (\cite{O'Hara2005}). The resultant multi-aged stand is suitable for achieving a variety of objectives including timber production, aesthetics, and restoring presettlement stand structures (\cite{O'Hara2005}). Nearly all of the aforementioned systems include a retained overstory component which is achieved through partial retention harvest. 
...
The common regional model of small tree growth, the Forest Vegetation Simulator (\cite{Dixon2013a}), introduces within-tree variability in height growth increment through assigning average growth from three different intervals within an assumed normal distribution about a mean height increment prediction. However, considering the skewed response distribution of small tree growth, predicting the mean height increment against the covariates may not be as informative as considering the full distribution of responses.

\subsection{Objectives}

Looking beyond the predicted mean height growth increment, the objectives of this research were to
\begin{enumerate}
...
\item{Relate findings to the common regional modeling methodology for small tree growth}
\end{enumerate}
