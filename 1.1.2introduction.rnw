...
Today's standard model of small tree growth, the Forest Vegetation Simulator, introduces within-tree variability in height growth increment through assigning average growth from three different intervals within an assumed normal distribution about a mean height increment prediction. However, considering the skewed response distribution of small tree growth, predicting the mean height increment against the covariates may not as informative as considering the full distribution of responses.
...
\item{Relate findings to the standard regional modeling methodology for small tree growth}
\end{enumerate}
