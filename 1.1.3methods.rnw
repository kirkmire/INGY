...
@
\end{center}
\caption{Site Index and Overstory Basal Area at Initiation}
\end{figure}
...
\par Only the installations with greater than 60 ponderosa pine tagged small trees at initiation will be included in the model \ref{numb.pond}.  Within these installations, the 6th small tree plots will be withheld as validation data. The check plots and installations that have sustained a post-initiation harvest are also excluded from analysis.  

\begin{figure}
\begin{center}
<<label=numb.pond, fig=TRUE,width=4,height=4,echo=FALSE>>=
#Frequency Plot by Species#
...
barchart(pipond$Freq~pipond$Installation,
        ...
        main=list(label="Tagged Ponderosa Pine at Initiation", cex=1.5)) 

@
\end{center}
\caption{Number of Small Tagged Ponderosa Pine Trees}
\end{figure}
