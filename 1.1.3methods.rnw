...
xyplot(sinst1$SiteIndex_Value~sinst1$minOSBA,
        xlab=list("Retained Overstory Basal Area (Basal Area Per acre)",cex=.8),
        col="white",grid=T,cex=.001,
        ylab=list("Site Index (P.Ponderosa) ft. ht. at 50 yrs.",cex=.8),
          main="",
        panel=function(x, y, ...) {
               panel.xyplot(x, y, ...);
               ltext(x=difInst$minOSBA, y=difInst$SiteIndex_Value, 
                     labels=difInst$Installation, offset=0, cex=0.8, col="black")
        #     },
        # panel1=function(x, y, ...) {
               panel.xyplot(x, y, ...);
               ltext(x=simInst$minOSBA, y=simInst$SiteIndex_Value,
                     labels=simInst$Installation , offset=0, cex=0.8, col="red3"
                     )
            }) 
...
The primary objective of the herbicide treatments is to decouple the site quality and vegetative effects. It has been reported that like tree growth increases in good sites, levels of
understory increase as well (Stage and Boyd 1987, Walstad and Kuch 1987). The herbicide treatments allow us to see how small trees grow under varying levels of site quality without the presence of  corresponding non-tree vegetation.)
...
\end{figure}
\text{Squares are years of understory and small tree measurements. Green and red triangles represent years of overstory measurements and herbicide treatment, respectively. Note that year the initial measurement periods selected such that they follow herbicide treatments.}
...
@
\end{center}
\caption{Design of STCV Sampling Plot. Note that transect points and quadrants exist for all six small tree plots although only illustrated on two.\label{fig:samplePlot}}
\end{figure}

Each plot contains a series of nested plots that decrease in area with physiologically smaller vegetation units (see Figure \ref{fig:samplePlot}). Starting with the full extent of the plot, overstory trees (> 10.5 in DBH) were measured on an approximately half acre. Medium trees, with DBH greater than 3.5 in but less than 10.5 in were measured on a smaller nested plot of roughly a quarter acre.
...
Only the installations with greater than 60 ponderosa pine tagged small trees at initiation will be included in the model (see Figure \ref{numbPond}). At these installations, the response variable; annual height increment, was calculated by finding the appropriate measurement intervals according to the timeline in Figure\ref{fig:timeline} on page \pageref{fig:timeline}, then subtracting earlier height increments from the latter and then dividing by the difference between measurement years. 


It was also during this stage of preparation that damage codes were addressed. Over the years many of the small tagged trees endured some kind of damage (see figure \ref{damageCodes}) including mortality, broken tops, forked tops, sweep or animal damage. Dead trees and those with dead tops were removed from analysis since these trees typically experienced a decrease in height growth from the previous measurement year. The histogram of annual height increments in figure *** shows that this effectively removes negative height growth increments.  See ***appendix for the complete list of damage codes recorded. 

\begin{figure}
\begin{center}
<<label=damageCodes, fig=TRUE,width=8,height=3,echo=FALSE>>=
