...
With the the ability to predict quantile annual height increments, we can predict the quantile height increments for new sets of data and thus have an idea of what the fastest growing small trees will advance in terms of an annual height increment.\\[2pt]
...
\par Only the installations with greater than 60 ponderosa pine tagged small trees at initiation will be included in the model \ref{numb.pond}.  Within each plot, one of the sixth STPs was randomly selected as validation data. The randomness was neccesary to account for the uphill orientation of the first STP on each plot and clockwise layout of remaining STPSs. The check plots and installations that have sustained a post-initiation harvest are also excluded from analysis.  

\begin{figure}
\begin{center}
<<label=numbPond, fig=TRUE,width=4,height=4,echo=FALSE>>=
#Frequency Plot by Species#
...
barchart(pipond$Freq~pipond$Installation,
        ...
        #main=list(label="Tagged Ponderosa Pine at Initiation") 
...
The \texttt{quantreg} package by Roger Koenker \cite{Koenker2015} will be used to fit quantile regression surfaces individually for the specified quantiles (specified as $\tau$ values: .90, .50, .10).  This allows for differences in the selected predictor variables between the quantile planes.
