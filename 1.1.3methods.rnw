...
volume(\frac{ft^{3}}{ft^{2}}) =  [\frac{ft^{2}veg}{ft^{2}land}]*[top-base]
\end{align}
where $volume$ is the per area volume estimate, $veg$ is the ocularly estimated precentage cover of vegetation, $land$ is one square meter vegetation sample plot, $top$ is the vegetation top height in feet and $base$ is the vegetation base height in feet.
