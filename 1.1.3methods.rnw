...
\text{where~\emph{y} is the growth response,\emph{s} is the slope in percent,~\emph{el} is elevation in feet,~\alpha is aspect and the \emph{b}, are coefficients estimated from the data}\nonumber
\end{gather}
