...
A recently developed class of models called linear quantile mixed models (LQMMs, \cite{marcoLQMMart}) extend quantile regression framework to models with random intercepts. These models are based on the asymmetric Laplace (AL) likelihood and will be used to account for the correlation between repeated measures of individual subject trees. A random intercept is included in the linear predictor to account this correlation and the rest of the selected variables are considered as fixed effects. The optimization algorithm utilizes a derivative-free minimization of the negative integrated log-likelihood. The parameters of this optimization will be set to a ${\omega_1}$=.1, number of max iterations set to 1000 and the number of quadrature points set to 100. 
