...
\begin{figure}
\begin{center}
<<label=siteOverstory, fig=TRUE,width=4,height=3,echo=FALSE>>=
...
understory increase as well (Stage and Boyd 1987, Walstad and Kuch 1987). The herbicide treatments allow us to see how small trees grow under varying levels of site quality without the presence of  corresponding non-tree vegetation.)\\[2pt]
...
Only the installations with greater than 60 ponderosa pine tagged small trees at initiation will be included in the model (see Figure \ref{numbPond}).  Within each plot, one of the sixth STPs was randomly selected as validation data. The randomness was neccesary to account for the uphill orientation of the first STP on each plot and clockwise layout of remaining STPSs. The check plots and installations that have sustained a post-initiation harvest are also excluded from analysis.  
