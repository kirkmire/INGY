...
The lower dbh limit imposed by the study ensured that only overstory trees greater than 3.5 in were included in the TPA metric, whereas a more traditional TPA would consider trees of all size classes. This likely resulted in an improved correlation between TPA and CAPA ((R=.92), see Figure, \ref{fig:capaTpa}). It is not entirely clear why TPA provides a better model fit than CAPA, but it is possible that changes in CAPA across measurement years somehow fail to relate to small tree height growth increment as well as decreases in TPA. TPA certainly provides a more practically obtained measure of overstory competition.
...
\begin{figure}[ht]
\begin{center}
    \includegraphics[width=160mm]{customFVSdist.png}
