...
\Delta{h} &=\propto_{1}h^{\propto_{z}}e^{\propto_{3}CCF}e^{\propto_{4}BAL}f(HAB,sl,\alpha)\qquad, 
\end{align}
where $h$ is tree height, $HAB$ is a habitat type dependent intercept, $CCF$ is crown competition factor, $BAL$ is total basal area in tree larger than the subject tree, $\alpha$ is the stand aspect, and $sl$ is the stand slope.
