...
%Model validation
One unanticipated result of using quantile regression to examine all parts of the response distribution was that 49 validation trees were predicted to have slightly negative $\tau$=.1 growth because the predicted height growth was not constrained to be strictly positive. The few trees that were prdicted to have a negative height growth were very short (average initial height of 2 ft), exhibited short crowns (average crown ratio of 19\%) and were found on sites with very dense small tree competition (KC, TP, LF). These negative estimates of growth likely occurred as a consequence of fitting linear quantile surfaces to predictors that had nonlinear effects on height growth near the lower limit. These surfaces had to be parameterized in such a way that they described the growth of the specified $\tau$ across all values. For the .10 quantile prediction, this meant that the surface dropped below zero for some combination of predictors. This explains why so few trees in the 1-5 ft height class exhibited height below their .10 quantile prediction.
...
Although there have been no other studies that predict height growth of small ponderosa pine trees with quantile regression, we can qualitatively compare our results to those of other empirical models. We can also compare the effects of our selected predictors to studies that examine specific competitive effects on small ponderosa pine growth.

%Need to discuss literature from intro in this subsection
%Curtis_lynch
Although several authors mentioned in the introduction found that vegetative competition reduced the height and diameter growth of ponderosa pine (\cite{barrett_1979}, \cite{curtis_lynch}, \citeyear{Busse1996}, \cite{oliver_1979}), we found no suitable measure of vegetative competition that relates to height growth. However, the competitive effect of overstory trees described by (\cite{Oliver1984}, (\cite{Ffolliott1982} and \cite{Moore1992})) was observed in all overstory measures and most pronounced in the measure of trees per acre.
...
\subsection{Qualitative Comparison to the Forest Vegetation Simulator Inland Empire Variant Small-Tree Height Increment Model}
...
The small-tree routine in the Inland Empire Variant of FVS provides a 5-year estimate of individual height growth (\cite{keyser2008}). It utilizes the following equation to predict the height growth of small trees (less than 3 in DBH):
%
\begin{align} \label{eq:fvs_eq}
\Delta{h} &\propto h^{B_{1}}e^{B_{2}CCF}e^{B_{3}BAL}f(HAB,LOC,sl,\alpha)\qquad, 
\end{align}
where $CCF$ is crown competition factor, $BAL$ is total basal area in trees larger than the subject tree, $HAB$ is habitat type, $LOC$ is a location modifier, $\alpha$ is the stand aspect, and $B_1$-$B_3$ parameters and $f$() a function being species-specific.
