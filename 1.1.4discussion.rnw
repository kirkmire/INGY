...
Higher rates of growth are observed in trees that have greater crown ratios and intitial heights. Once again, quantile regression provides a technique to describe the maximum growth for a given crown ratio and height (by specifying $\tau$=.9). The central and lower regions of this distribution are also described by the quantile regression models ($\tau$=.5 and .1, respectively).
...
\end{align}
where $CCF$ is crown competition factor (\cite{Krajicek1961}), $BAL$ is total basal area in trees larger than the subject tree, $HAB$ is habitat type, $LOC$ is a location modifier, $\alpha$ is the stand aspect, and $B_1$-$B_3$ parameters and $f$() a function being species-specific.
...
FVS does accommodate multiple trajectories of tree records to provide some degree of variation from the mean. When relatively few samples represent the stand, two additional tree records are created that triplicate the characteristics of the tree except the number of trees per acre represented. The two new records represent 15 and 25 percent of the trees per acre and the original record represents 60 percent of trees per acre. A normally distributed random error bounded by -1.5 and 1.0 standard deviations is then added to each of the triplicated tree records. 
