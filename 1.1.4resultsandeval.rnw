...
The total number of small tagged ponderosa pine trees at initiation is reflective of how many tree records each installation provides. Figure \ref{fig:numbPond} shows that when multiple growth intervals and the number of tree records excluded due to mortality or damage are accounted for, many installations contribute over 150 tree records to the model, while installations with only a single measurement interval (RM and TJ) actually contributed fewer than 60 tree records. There were five installations that had tagged ponderosa pine tree records in excess of 200 and two that had over 300 (EM and LF). 
