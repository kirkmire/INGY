...
Table is related \ref{tab:tree.counts} to Figure \ref{fig:numbRecords} in that it reveals the number of times a unique tree contributed to the model. A nearly equal amount of trees contributed to the model over either one, two or three measurement intervals. Ultimately, the majority of the tree records come from trees that were included over three intervals.
...
%   \subcaption{Annualized height growth increment (ft) vs. crown ratio.}\label{fig:delta_ht_cratio}
% \endminipage\hfill
% \minipage{0.48\textwidth}
