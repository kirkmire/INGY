...
<<label=distinctrecords,echo=FALSE,results=tex>>=
library(xtable)
remeas.struct <- data.frame(obs=c(2,3,4),
           trees=as.numeric(table(table(annual.gr4$conc))))
remeas.struct$rec <- (remeas.struct[,1]-1)*remeas.struct[,2]
colnames( remeas.struct ) <- c( "Number of measurements", 
                                "Number of trees", 
                                "Number of growth records" ) 
print(xtable(remeas.struct,
       caption="Number of tree records from each unique tree.",
       label="tab:tree.counts",
       digits=0),
      include.rownames=F)
@
...
@
\end{center}
\caption{Small tree records removed due to top damage or mortality over time.\label{fig:damageCodes}}
\end{figure}
...
@
 \end{center}
 \caption{Species composition of small tree plots in installations with more than 60 tagged ponderosa pine at initiation. PSME= \emph{Psuedotsuga menziesii}, PIPO= \emph{Pinus ponderosa}, ABGR= \emph{Abies grandis}, THPL= \emph{Thuja plicata}, PIMO= \emph{Pinus monticola}, PICO= \emph{Pinus contorta}, LAOC= \emph{Larix occidentalis}.\label{fig:speccomp}}
 \end{figure}
...
@
\end{center}
\caption{Plot vegetation volumes by herbicide treatment at the start of the measurement period (GE = multiple applications, 1X = one time application, CTRL = no herbicide).\label{fig:vegVol}}
\end{figure}
...
\endminipage\hfill
\caption{Standardized $\tau$=.5 predicted growth increment by slope (\%) and elevation (m). Lighter shades indicate a greater predicted height growth increment. Initial height= 1ft, crown ratio=.5, overstory TPA=40.}\label{fig:Sea_heat}
\end{figure}
...
%                         elevation=rep(seq(600,1350,length=50),each=50))
