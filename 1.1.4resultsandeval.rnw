...
print(barchart(pipond$Freq ~ pipond$Installation,
         ...
         xlab="Installation", ylab="Number of Tagged Small Ponderosa Pine Trees by STCV ",
...
@
 ...
 Out of the 29 installations, only 15 had sufficient numbers of ponderosa pine tagged trees to be considered 
 in this analysis. This threshold number of tagged ponderosa pine limits the analysis to installations with a significant 
 ponderosa pine understory component.  Based on the frequencies of small trees by species and installation in Figure \ref{fig:speccomp}, the selected installations are not dominated by Douglas-fir (Pseudotsuga men.) regeneration although there are two installations that are very closely split between the two species (LF and DF).
...
The number of tallied small trees varies widely by installation and small tree plot. Keifer Cabin (KC), Loon Lake (LL) and (TP) had dense regeneration. 
...
@
\end{center}
\caption{Plot vegetation volumes by herbicide treatment (GE = multiple applications, 1X = one time application, CTRL = noherbicide)\label{fig:vegVol}}
\end{figure}
...
Overstory basal area per acre was calculated from the initial overstory sample plots. There appears to be a group of six installations with similarly low overstory retention levels and high site index values (see red colored installations in Figure \ref{fig:SI.vs.BAPA}) on page \pageref{fig:SI.vs.BAPA}. These installations were used to develop the understory model since they were so alike in overstory and site productivity. 
...
Most subject trees experience annual height growth between 0 and 2 feet (Figure \ref{ht_by_inst}). The asymmetric, right skewed distribution reveals that very few of the subject trees grew more than 2 feet annually.
 
The initial height of small ponderosa pine trees ranges between 1 foot (the minimum height to be included in the sample) to just under 30 feet with the majority of tagged trees are between 1 and 15 feet in initial height.
...
Figure \ref{fig:os_tpa_inst} shows the plot-level interpolations of overstory trees per acre. Most plots stayed constant in the number of overstory trees. However, due to mortality or recruitment, there may be slight differences. Within installations, there is noticable variation in overstory trees per acre between plots. This is usually advantageous in model-building as it provides a range of overstory conditions within each installation. However, due to the understory treatments, a homogenous overstory condition is favorable to isolate the understory effects. Fortunately, for most installations the plot levels are similar (LF seems to be the exception).
...
Although generalized additive models (GAMs) were evaluated for every variable to check for any non-linear relationships, only those of the selected variables are shown below. The GAMs suggested no improvement in the relationship between annual height increment and a given variable via transformation. The included GAMS (Figures \ref{fig:seqgamCR},\ref{fig:seqgamOSTPA},\ref{fig:seqgamSEA}) show a strong linear relationship between 
