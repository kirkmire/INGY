...
The total number of small tagged ponderosa pine trees provided by each installation varied substantially (Figure \ref{fig:numbPond}). When multiple growth intervals and the number of tree records excluded due to mortality or damage are accounted for, many installations contribute over 150 tree records to the model, while installations with only a single measurement interval (RM and TJ) actually contributed fewer than 60 tree records. There were five installations that had tagged ponderosa pine tree records in excess of 200 and two that had over 300 (EM and LF). 
...
@
\end{center}
\caption[Crown area per acre vs. trees per acre by plot.]{Crown area per acre vs. trees per acre by plot. The measurements within each plot across years are represented by a common line and color. Any middle values are interpolated from overstory measurements taken at the beginning and end of the measurement interval.\label{fig:capaTpa}}
\end{figure}

Figure \ref{fig:capaTpa} shows that crown area per acre (CAPA) and trees per acre (TPA) variables are closely correlated (r=.92). Variation in trees per acre within plots is attributed solely to mortality therefore any change in trees per acre across measurement years is negative. Crown area per acre may increase or decrease depending on the growth of crowns in overstory trees and the loss of crown due to mortality. Many plots in the intermediate TPA range (40-70 TPA) show a lack of overstory mortality and an increase in crown area per acre. However, plots with higher residual overstory TPA tend to exhibit higher levels of mortality and a corresponding decrease in crown area per acre.
...
\endminipage\hfill
\caption{Partial residuals plots of annualized height growth vs. initial height and crown ratio.}
\end{figure}
...
\minipage{0.48\textwidth}
  ...
  \subcaption{Partial residuals of annual height growth vs SEA.}\label{fig:seqgamSEA}
\endminipage\hfill
\caption{Partial residual plots of annualized height growth vs. TPA and SEA.}
\end{figure}
...
% 
% with(dat, plot(elevation,growth9,type="l",col="green",
% xlab="Elevation (ft)",ylab= "Annual Height Growth (ft)"))
% with(dat, lines(elevation,growth5,type="l",col="orange"))
% with(dat, lines(elevation,growth1,type="l",col="red"))
...
% 
% with(dat, plot(aspect_degr,growth9,type="l",ylim=c(0,1.0),col="green",
% xlab="Aspect (Degrees N)",ylab= "Annual Height Growth (ft)"))
% with(dat, lines(aspect_degr,growth5,type="l",col="orange"))
% with(dat, lines(aspect_degr,growth1,type="l",col="red"))
...
\endminipage\hfill
\caption[Predicted annual height growth by aspect and elevation.]{Predicted annual height growth by aspect and elevation. Green, yellow and red lines represent the .9, .5 and .1 $\tau$ regression quantiles, respectively. Initial height set to 1 foot, crown ratio set to .5, TPA is 40 and slope is 10\%.}\label{fig:sea_gr}
\end{figure}

Figures \ref{fig:gr_asp} and \ref{fig:gr_elev} show the effects of slope and elevation among quantile etimates of annual height growth when all other variables are held constant. In Figure \ref{fig:gr_asp}, the effect of aspect increases as the $\tau$ value increases. However, the differences in height growth estimates among quantiles by apect may change at different elevations or slopes due the interaction between these terms. Aspect is shown to increase annual height growth at an optimum of approximately 200$\degree$ when the slope is 10\% and the elevation is 3,500 ft. 

                      
                          
Figure \ref{fig:gr_elev} shows that elevation is estimated to have a positive effect on annual height growth for most elevations, although, for lower elevations where there is sparse data, elevation is estimated to have a negative contribution to annual growth. The magnitude of the elevation effect increases across quantile $\tau$ values for commonly observed elevation conditions. This means that the effect of elevation is greater for the fastest growing trees for the given parameters. However, as in Figure \ref{fig:gr_asp}, the effect of elevation may change for different slope and aspect values due to the interaction between these predictors.  The optimum elevation appears to be around 3500 ft for all $\tau$ values when the aspect is 200$\degree$ and the slope is 10\%.                    
