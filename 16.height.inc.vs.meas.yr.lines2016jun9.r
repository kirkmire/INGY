...
quantile(LL2012$inc, probs = c(.25, .5, 0.75))


LL1_both$qu<-LL2012$q[match(LL1_both$Plot,LL2012$Plot)&&
                        match(LL1_both$Tree,LL2012$Tree)]




lllp<-ggplot(data=LL1_both, aes(x=Year_Measurement.y, y=inc,color= factor(LL1_both$qu)))+geom_point()+
  labs(title="LL Tree H-H Inc over Meas Years",y="Height-Init Height (ft)", x = "Measurement Year")+ 
  scale_fill_brewer(palette="Dark2") +tbp+ geom_line(aes(group=LL1_both$Tree))+ylim(0,20)
