...
Only the installations with a greater than 100 Ponderosa Pine tagged small trees at initiation will be included in the model.  Within these installations, the 6th plot will be withheld as training data. The check plots and installations that have sustained a post-initiation harvest are also excluded from analysis.  \\[2pt]

The "quantreg" package by Roger Koenker cite(Koenker) will be used to fit quantile regression planes individually for the specified quantiles (specified as tau values: .90, .50, .10).  This allows for differences in the selected predictor variables between the quantile planes.  \\[2pt]
...
To evaluate the utility of higher-order polynomials, a Generalized Linear Model (GLM) with a smoother of all first-order predictors will be used to view the relationship with the predictors individually. If inclusion of a higher-order predictor is justified by the GLM plot than it will be considered alongside all other categorical predictors and as an interaction term.\\[2pt]
...
\item{Create a list of the practical predictor variables within each category:
 ...
 \end{itemize}
  ...
  \item{For each candidate variable in the category ``understory tree'' generate a generalized additive model (GAM) using the base model:
  \begin{center}
...
\end{equation}
  ...
  \item{Repeat steps 5-10 for the understory nontree vegetation variables}
  \item{Repeat step 11 for Tau=.90 and .10}
  \item{Select variables from the overstory and site productivity categories using the process outlined in steps 5-12 using all installations selected in step 1}\\
  \textbf{Final Models:}
      \begin{center}
