...
\textbf{Variable Selection Process:}\\[1pt]
\begin{enumerate}
  \item{Fit quantile regression models for $\textrm{ Tau}=.5$ using the base model of
  \begin{center}
\begin{equation}\label{eq:Base Model}
  \Delta \textrm{ height annual}=\sqrt{\textrm{initial height}}+\textrm{ small tree variable}
\end{equation}
  \end{center}}
  \item{}
\end{enumerate}
