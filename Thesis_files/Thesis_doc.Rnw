...
Composition of forest understory species are controlled by overstory trees which filter light, moderate understory air and soil temperature,and diretly compete for soil water and nutrients cite{need Spurr and Barnes 1980}.  

\begin{wrapfigure}{r}{\textwidth}
\begin{center}
    \includegraphics[width=50mm]{comp_flowchart.jpg}
...
\begin{wrapfigure}{r}{\textwidth}
\begin{center}
    \includegraphics[width=50mm]{inst_map.png}
    \caption{Map of Installations (with > 60 small tagged Ponderosa Pine)}
    \label{fig:mesh1}
\end{center}
\end{wrapfigure}
...
spec_freq<-barchart(speciescount$freq~speciescount$x,
...
si_matrix<-xyplot(threeDVeg$SiteIndex_Value~threeDVeg$ave.BAPA, type=c("g","p"),
...
The site productivity of the installations was measured according to the cominate site tree which varied between ponderosa pine, douglas fir and ... Selecting only stands with greater than 60 small tagged ponderosa for this analysis effectively removed nearly all stands that meaasured site index with a species besides ponderosa pine.
