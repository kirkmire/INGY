...
\begin{enumerate}
  \item{Fit quantile regression model for $\textrm{ Tau}=.5$ using the base model of
\begin{center}
  $\triangle \textrm{ height annual}=\sqrt{\textrm{initial height}}+\textrm{ small tree variable}$
\end{center}}
  \item{another cat}
\end{enumerate}
