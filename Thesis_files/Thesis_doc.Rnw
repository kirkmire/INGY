...
<<speccomp, include=false,fig=TRUE,width=4,height=4,echo=FALSE>>=
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=160mm]{Thesis_doc-speccomp}
...
png("inst_map.png",width=150*10,height=150*10*ratio,pointsize=8,res=400)
...
#tmp <- PlotOnStaticMap(basemap,cex=2,pch=21,col="red",
                      # lat=locs0v2$Lat,
                       #lon=locs0v2$Long)
#For text labels
tmp <- PlotOnStaticMap(basemap,cex=.9,col="black",add=T,
lat=locs0v2$Lat,
lon=locs0v2$Long)
#FUN=text,
...
\begin{figure}[ht]
\begin{center}
    \includegraphics[width=150mm]{inst_map.png}
...
<<sampleplot,include=FALSE,fig=FALSE,width=5,height=5,echo=FALSE>>=
library(plotrix)
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=80mm]{Thesis_doc-sampleplot}
...
<<species,include=False,fig=FALSE,width=5,height=5,echo=FALSE>>=
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=80mm]{Thesis_doc-species}
...
\begin{figure}[ht]
\begin{center}
<<include=TRUE,fig=TRUE,width=5,height=5,echo=FALSE>>=
library(plyr)
library(data.table)
library(lattice)
...
OverS <- merge(min.OS, soverhist5,
               by=c("Installation","Plot","Tree","Year_Measurement"))

OverS$BAPA<-ifelse(OverS$DBH<10.5,((OverS$DBH^2)*.005454)/.26,
                   ((OverS$DBH^2)*.005454)/.46) 

BAPA.by.plot<-aggregate(OverS$BAPA, 
        by=list(Installation=OverS$Installation,Plot=OverS$Plot), FUN=sum)
...
CTRLandGE.volume.index<-aggregate(CTRLandGE.plot.veg[, 17:19], list(Installation=CTRLandGE.plot.veg$Installation,Trt=CTRLandGE.plot.veg$Treatment), mean, na.rm=TRUE)
...
simatrix<-xyplot(threeDVeg$SiteIndex_Value~threeDVeg$ave.BAPA, type=c("g","p"),
       groups=threeDVeg$SiteIndex_Species,
       xlab=list(label="Overstory Basal Area (ft^2/ac)",cex=1.5),
        ylab=list(label="Site Index Value",cex=1.5),
        main=list(label="Site Index and Residual Basal Area Matrix", cex=1.5),
        par.settings=list(superpose.symbol=list(
                      pch=c(15,16),
                      col=c("blue","red","green","purple"))))
print(simatrix)
@
\caption{Matrix of Site Index and Overstory Conditions}
\label{fig:si_os_matrix}
\end{center}
\end{figure}
...
CTRLandGE.volume.index<-aggregate(CTRLandGE.plot.veg[, 17:19], list(CTRLandGE.plot.veg$Installation, CTRLandGE.plot.veg$Treatment), mean, na.rm=TRUE)

#Create a US Volume Estimate by multiplying mean ployveg% cover * mean top height for each treatment w/i each inst#
CTRLandGE.volume.index$volume<-CTRLandGE.volume.index$Cover*CTRLandGE.volume.index$Top

CTRLandGE.volume.index<-CTRLandGE.volume.index[! CTRLandGE.volume.index$Group.1 %in% drp,]
...
barchart(CTRLandGE.volume.index$volume~CTRLandGE.volume.index$Group.1,groups=CTRLandGE.volume.index$Group.2,
         ...
         main=list(label="Vegetation Volume at First Measurement Year", cex=1.5),
                   auto.key=T)
@
\caption{Volume Comparison of Understory Vegetation in First Year of Timeline}
\label{fig:veg_vol_yearone}
\end{center}
\end{figure}
...
\subsection{Fitting Quantile Regression}


Only the installations with a greater than 60 Ponderosa Pine tagged small trees at initiation will be included in the model.  Within these installations, the 6th plot will be withheld as training data. The check plots and installations that have sustained a post-initiation harvest are also excluded from analysis.  \\[2pt]


The variable selection process for each individual quantile will be guided by the ecological framework behind small tree growth.  This means that the predictor variables will be selected from the four previously mentioned categories of ecological factors that affect tree growth. These are overstory measures, understory non-tree vegetation, site productivity and other small tree measures.  The square root of initial height (height at the beginning of measurement period) will also be included as a predictor of height growth increment in the "base model" since it is clear that square root of initial height explains much of the variability in height growth increment and we are interested in the other predictors.\\[2pt]

Variable selection from within these categories is analogous to that of least squares, except that variables will be selected with respect to their significance to the regression of the median (Tau=.5). Within each category of ecological factor, a subset of relevant variables will be considered that includes two-way interactions between the previously selected variables and the category under evaluation. This subset of variables will also take into consideration the practicality of the predictors.\\[2pt]

To evaluate the utility of higher-order polynomials, a Generalized Additive Model (GAM) with a smoother of all first-order predictors will be used to view the relationship with the predictors individually. If inclusion of a higher-order predictor is justified by the GLM plot than it will be considered alongside all other categorical predictors and as an interaction term.\\[2pt]

The "quantreg" package by Roger Koenker cite(Koenker) will be used to fit quantile regression planes individually for the specified quantiles (specified as tau values: .90, .50, .10).  This allows for differences in the selected predictor variables between the quantile planes.
Model building will proceed from category to category with the model of lowest AIC being selected as the model carried forward to the next category. This will result in a parsimonious model that is easily understood and supported by our understanding of the factors surrounding small tree growth.\\[2pt]

The order of variable selection categories will proceed in the following order; understory tree, understory non-tree vegetation, overstory and finally site. The first two variables;understory non-tree, and understory small tree will be selected from the control plots of a group of installations of comparable understory conditions. Once an ``understory model'' is established, the selection of overstory and site variables will proceed in that order.  These two final categories necessitate the ``linking''of all selected Ponderosa Pine installations \\[2pt]
...
\begin{small}
  ...
  \item{Retain the sixth plot of each installation for validation}
  
\item{Create a list of the practical predictor variables within each category:
  \begin{itemize}
         \item {Understory Tree}
         \item {Understory Non-tree}
         \item {Overstory Tree}
         \item {Site Productivity}
 \end{itemize}
}
  
  \item{For each candidate variable in the category ``understory tree'' generate a generalized additive model (GAM) using the base model and only the control plots (no herbicide):
  \begin{center}
...
\end{equation}
  ...
  \item{Visually examine the results and residuals of the models produced in 5}
  
  \item{Include quadratic terms of variables in the list of candidate variables if warrented}
  
  \item{Fit a quantile regression model with each candidate variable for$\textrm{ Tau}=.5$ using the quantreg package
    \begin{center}
...
\end{equation}
  ...
  \item{Calculate the AIC for each model within the small tree category}
  
  \item{Select the variable from the model with the lowest AIC to be carried forward into the "understory nontree vegetation" category}
  
  \item{Repeat steps 5-10 for the understory non-tree vegetation variables}
  \item{Repeat step 11 for Tau=.90 and .10}
  \item{Select variables from the overstory and site productivity categories using the process outlined in steps 5-12 using all installations selected in step 1 (and all plots)}\\
  \textbf{Final Models:}
      \begin{center}
\begin{equation}\label{eq:Eq3}
\begin{aligned}
 \emph{Q}_{.10,.50,.90} \Delta \textrm{ height annual}=\sqrt{\textrm{ initial height }}+\textrm{ small tree variable }\\
+ \textrm{ understory variable }+\textrm{ overstory variable }+\textrm{ site variable}
  \end{aligned}
  \end{equation}
  \end{center}
\end{small}
