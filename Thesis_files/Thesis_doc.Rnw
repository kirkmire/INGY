...
\end{equation}
  ...
  \item{Using the variables selected for the $\textrm{$\tau$=.5}$, fit quantile regressions for $\textrm{$\tau$=.1,.9}$. This will estimate different \textrm{b}_{\tau} for each specified quantile.\\
  \textbf{Final Models:}
